...
const getTodo = async (req, res) => {
    try {
        ...
        if (!todo) {
            return res.status(404).json({"success": false, "message":"Todo with id " + req.params.id + " does not exist"});
        } 
...
const updateTodo = async (req, res) => {
    try {
        ...
        if (!todo) {
            return res.status(404).json({"success": false, "message":"Toddo with id " + req.params.id + " does not exist"});
        }
